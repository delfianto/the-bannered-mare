...
All routes render inside `App.vue → AppShell.vue → RouterView`. The desktop shell uses `AppSidebar.vue`; routed pages usually use `PageContainer.vue`.

| URL | Name | View | Summary |
|---|---|---|---|
| / | home | frontend/src/views/HomeView.vue | Greeting, search, recent stories, character discovery |
| /chats, /chats/:chatId | chats/chat | frontend/src/views/chat/ChatView.vue | Session browser and immersive conversation reader |
| /characters | characters | frontend/src/views/CharactersView.vue | Filterable character library |
| /characters/create | character-create | frontend/src/views/CharacterCreateView.vue | Character authoring flow |
| /characters/:id | character-detail | frontend/src/views/CharacterDetailView.vue | Character profile and start-chat actions |
| /bookmarks | bookmarks | frontend/src/views/BookmarksView.vue | Saved sessions/messages |
| /memory | memory | frontend/src/views/MemoryView.vue | Data bank and retrieval context |
| /connections | connections | frontend/src/views/ConnectionsView.vue | Provider/model/preset/template management |
| /loadouts | profiles | frontend/src/views/ProfilesView.vue | Roleplay loadouts and personas |
| /setup | setup | frontend/src/views/SetupWizardView.vue | Guided first-run setup |
| /lorebooks | lorebooks | frontend/src/views/LorebooksView.vue | Lorebook management |
| /settings | settings | frontend/src/views/settings/SettingsView.vue | Interface, logs, and about settings |
