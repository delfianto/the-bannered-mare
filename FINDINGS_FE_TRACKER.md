...
### FE-M5 · Expose a stop button + fix regen-abort restore · [x] DONE — resolves FE-L-latent (see §Completed) · 🧵 main · dep: none · pairs-with: FE-L-latent
...
### FE-H2 · Move hardcoded English toasts/view text into vue-i18n · [x] DONE (a+b+c) (see §Completed) · 🤖 sub · dep: none
...
### FE-M1 / FE-M2 · One state-cache strategy (kill provider split-brain) · [x] DONE (see §Completed) · 🧵 main · dep: none
...
- **Commit:** `3939e5c` (FE-M1) · `e0b0bc8` (FE-M2, Option A)
...
- **[x] FE-L2** (commit `a3b561a`) — built a reusable async confirm dialog rather than forcing `useConfirmAction` (a two-click *inline* pattern, wrong for these sites). `useConfirm()` is a module-level singleton (like the toast store) returning `Promise<boolean>`; `ConfirmDialog.vue` renders the shared Modal from that state, mounted once in App.vue. Routed through it: `useLocalModelManagement` load/unload/delete (delete = danger + "Delete") and the `FragmentCreateView`/`TemplateCreateView` `onBeforeRouteLeave` unsaved-changes guards (router awaits the promise to allow/block nav; the native `beforeunload` stays for tab-close). Messages kept as literals (full i18n is FE-H3); added `common.confirm`. Gate: build, 61 tests, oxlint, tailwind.
- **[x] FE-M5** (commit `b239aa0`; resolves FE-L-latent) — **stop button:** ParchmentInput's send button morphs into a stop button while `generating` (square icon, emits `stop`); ChatView wires `:generating`/`@stop` to the already-exposed `stop()`. **Correctness (the entangled bug):** `readStream` was swallowing `AbortError` (returning quietly), so the callers' `if AbortError return` guards were dead code — `regenerate`'s catch never ran, and since it optimistically removes the prior reply, a stopped regen silently LOST it. Made `readStream` re-throw AbortError (sendMessage still returns quietly; regenerate now sees it) and `regenerate` refetches on abort (guarded on still being the same chat, so it doesn't fight the chat-switch watch) to restore the prior reply. Regression test drives regen→stop and asserts the reply is restored with no blank placeholder. Added `chat.stopGeneration` to en.json (de/es/fr/pt fall back — the FE-H3 gap). Gate: build, 61 tests, oxlint, tailwind.
...
- **[x] FE-M1 / FE-M2** (commits `3939e5c` FE-M1, `e0b0bc8` FE-M2) — unified the list-cache strategy on **Option A (shared Pinia singletons, the providers pattern)**.
  - **FE-M1:** the provider list is a store singleton, but `useProvider` create/save never touched it, so the shared list went stale unless each call site manually `fetchProviders(true)`. Moved invalidation into `useProvider.createProvider/saveProvider` and removed both manual call sites.
  - **FE-M2:** the other shared lists each auto-fetched per component instance (N consumers = N round-trips + N independently-mutated copies). Added **`stores/listStore.ts`** — a `defineListStore` factory (the singleton counterpart to `useListCrud`): fetch-once `ensureLoaded()` + cache, in-place `create/update/remove/setDefault`, `refresh()` for retry + invalidation. Migrated **presets, templates, personas, profiles** to store-backed composables (public API unchanged — verified by vue-tsc across all consumers). Profiles/personas mutate through the list composable → in-place coherence; the out-of-band single-item mutators invalidate the cache after writing (`usePreset` save/delete/setDefault, `usePromptTemplate` create/save/delete, `usePresetImport` import) — same self-invalidation pattern as FE-M1, centralized in the composable so no call site can forget.
  - **Deliberately left per-instance:** `useDataBank` — its `list` is scope-parameterized (scope/chat/character), which one global singleton would clobber; a keyed cache is the rejected Option B, not worth it for 2 consumers. `useListCrud` stays (now used only by databank). **Not covered by a cache/invalidation test** — matches the existing bar (providers/FE-M1 have none either); behavior is type-checked + the pattern mirrors the proven provider path. Gate: build, 69 tests, oxlint, fmt:check, both tailwind lints.
...
- **[x] FE-H2 — DONE (a toasts + b SetupWizard + c settings-detail)** (commits `FE-H2` (a), `6904e10` (b/c)) —
  ...
  - **Verified (b/c):** a script checked all **198** `t()` refs resolve in en.json (0 missing); two prose-scan greps → 0 leftover literal text nodes/attributes; build (vue-tsc → keys typecheck), 61 tests, oxlint, fmt, both tailwind lints green. de/es/fr/pt still fall back to English → **FE-H3**.
  - **Lint-guard skipped (deliberate):** `vue-i18n/no-raw-text` needs `@intlify/eslint-plugin-vue-i18n` + `vue-eslint-parser` (an ESLint/Vue stack this repo doesn't run — oxlint only) and false-positives on inline punctuation. Too much machinery for a local app; regression risk accepted.
