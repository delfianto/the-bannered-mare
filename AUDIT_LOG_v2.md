...
> **Status (2026-07-14, final):** **all 26 findings fixed, committed & (where observable) browser-verified.** Tier A, B, D complete; C3/C4/C5 complete (C5's "DTO not viable" was re-checked and corrected — a Form-model spreads when the `UploadFile` is inside it); **C2** complete (duplicate-lorebook-sync removed + `ChatHeader` slimmed from 10 props/7 emits to 2/2, both mock-verified); **C1** complete (the 1067-line ChatDrawer split into a 113-line shell + 4 lazy tab children + `useOverlayTransition` + tested `formatLog.ts`, all four tabs mock-verified). Legend: `[x]` done.
...
| V2-C1 | C | FE | High | `ChatDrawer.vue` 1087-line god component + monoliths | [x] |
| V2-C2 | C | FE | Med | business logic stranded in views; `ChatHeader` relay | [x] |
| V2-C3 | C | FE | Low | `useBookmarks` raw fetch, no `.ok`, `any[]` | [x] |
| V2-C4 | C | FE | Low | overlapping lint stacks + doc drift | [x] |
| V2-C5 | C | BE | Low | no shared character DTO; no async `get_or_404` | [x] |
...
### V2-C1. `ChatDrawer.vue` is still a 1087-line god component  ·  High (cohesion) · FE  · [x] DONE
...
### V2-C2. Business logic stranded in views; `ChatHeader` prop-drilling  ·  Med · FE  · [x] DONE
...
### V2-C4. Overlapping lint stacks contradict the docs  ·  Low · FE  · [x] DONE
...
### V2-C5. BE-21 residuals  ·  Low · BE  · [x] DONE (re-checked — DTO **is** viable)
...
- **[x] V2-C5 (character DTO — re-checked, my earlier "not viable" was WRONG)** — the blocker was a Form-model + a *sibling* `File()` param (FastAPI embeds it as a `data` field). Probes proved a Form-model **with the `UploadFile` field inside it** spreads correctly (with and without a file). Implemented: `CharacterFormBase` (17 fields, no `UploadFile`) declared **once** in schemas and consumed by both `service.create`/`update`; a router-only `CharacterFormPayload(CharacterFormBase)` adds `avatar: UploadFile`, and the router maps transport→DTO via `CharacterFormBase(**data.model_dump(exclude={"avatar"}))` — so `UploadFile` never reaches the service (BE-8 preserved) and `name`-required is enforced in `service.create` (clean 422, no pyright field-override pain). The ~17-field payload is now spelled out once instead of four times. Regenerated `openapi.json` (−259 lines) + `schema.d.ts`; frontend unaffected (hand-built FormData). Validated: ruff clean, basedpyright 0/0/0, 865 passed, 7 HTTP multipart router tests pass, `alembic check` clean, frontend build/lint/tests green. Commit: `6cd7d47`.
...
- **[x] V2-C1 (DONE + browser-verified)** — split the 1067-line `ChatDrawer` into a **113-line shell** (overlay + header + tab switch) plus four focused children, each owning its composables and lazy-fetching on activate (`v-if`): `ChatDrawerCharacterTab` (useCharacter), `ChatDrawerSettingsTab` (models/profiles/personas/dataBank/lorebooks + loadouts/persona/memories/lorebooks + rename/delete), `ChatDrawerSessionTab` (useChatPromptPreview), `ChatDrawerLogsTab` (useChatLlmLogs + completionSignal). Log formatters extracted to pure, unit-tested `utils/formatLog.ts` (+10 tests). `useOverlayTransition` (the V2-B5 deferred piece) extracted and adopted by the shell (commit `7174af3`); Modal keeps its own — its transition is coupled to focus-trap + stacked-modal scroll-lock. The shell relays the Settings tab's mutation emits to ChatView. Instance-level composable caches → each tab refetches on (re)activate (fine for a drawer). MSW verification: all four tabs render + lazy-load, applying a profile fires `POST /chats/{id}/profile 200` through the shell relay, no console errors. Commits: `7174af3` (useOverlayTransition), `882e5b3` (tab split). Validated: `bun run build`, fmt:check, `vp lint`, 18 tests.
- **[~] V2-C2 (partial)** — extracted `utils/download.ts::downloadJson` and wired `CharacterCreateView.handleExport` to it (commit `bd876a3`). The two larger parts are deferred to a spawned task (need mock-mode verification): (a) `CharacterCreateView.handleSave` **duplicates** the lorebook-sync loop that `useCharacterForm.saveCharacter` already performs internally — consolidating requires verifying create+edit to avoid breaking lorebook persistence; (b) `ChatHeader`'s 10-prop/7-emit relay should source `models`/`profiles`/`personas` from composables + a `useChatSession(chatId)` (best done with the C1 split). Validated (downloadJson): `bun run build`, fmt:check, `vp lint` pass.
  - **Update (re-check):** part (a) **DONE + browser-verified** (commit `d9f891f`). Deep-traced both sync paths: `useCharacterForm.saveCharacter` already does a *complete* sync (create/update/**delete**), while `CharacterCreateView.handleSave` ran a **second, delete-less** sync that read a possibly-unloaded `currentLorebook` → `isExisting` false → **re-created every entry (duplicate-write bug)**. `onMounted` likewise re-loaded a lorebook `loadFromApi` already loads. Removed all `loreApi` usage from the view (delegates to `useCharacterForm`). MSW mock-mode verification: create page renders clean (no console errors), empty name → 422, named character → **201 + exactly one `/api/lorebooks` sync (no second pass)** → navigates back. Only part (b) `ChatHeader` remains (spawned task `d3317b3d`).
  - **Update 2 (part b DONE + browser-verified, commit `62b479d`):** `ChatDrawer` now sources its own picker lists (`useModels` + `useProfiles`; personas already internal), dropping the `models`/`profiles` props; the drawer moved out of `ChatHeader` into `ChatView`, wired directly to the existing `handleChange*`/rename/delete handlers. `ChatHeader` is now presentational — `character` + `sessionTitle` props, `back` + `openMenu` emits (down from 10 props / 7 emits). Chose direct View→Drawer wiring over a `useChatSession` composable (which would have competed with `useChatSessions` for chat-mutation state). MSW verification: header renders, menu opens the drawer, Settings tab shows the self-sourced model (GPT-4o) + 4 profiles, applying a profile fires `POST /chats/{id}/profile 200` + refreshes the list, no console errors.
