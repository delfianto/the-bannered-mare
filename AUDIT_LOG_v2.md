...
> **Status (2026-07-14, after partial re-check):** 25 of 26 fixed & committed (all Tier A, B, D, plus C3/C4/**C5**). **V2-C5** was re-checked and completed — my earlier "DTO not viable" was wrong (a Form-model spreads if the `UploadFile` is *inside* it). **V2-C2** is now mostly done: the view-logic/duplicate-lorebook-sync half is fixed **and browser-verified** in mock mode; only the `ChatHeader` prop-drilling half remains (spawned task). **V2-C1** (ChatDrawer 1087-line split) is the one item deferred to a spawned task — a genuinely large structural refactor. Legend: `[x]` done · `[~]` partial · `[→]` deferred to spawned task.
