...
- **Updated:** 2026-07-15
- **Active:** 🔨 **BE-H2 Step 2 in progress.** `model` migrated (provider→`ReadPort[Provider]`, model_family→`ModelFamilyService`, `get_or_404` widened to `ReadPort[T]`); gates green (941). **To resume:** BE-H2 → Step 2, `prompt_template`→`prompt_fragment` (classify reads), then `chat_session` reads (coordinate w/ BE-H7); then Step 3 (writes via services) + Step 4 (import-boundary AST lint). (BE-H1 complete: UoW sync+async across all 17 services.)
...
- [~] **Step 2 — entangled reads.** Design: simple reads → thin `Port`; complex reads → target **Service**; foreign-session access → caller's OWN session.
