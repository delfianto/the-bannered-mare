...
- [ ] **Step 2 — entangled reads (analyzed; turnkey plan below).** Design decided: simple reads → thin `Port`; complex reads → target **Service**; foreign-session access → caller's OWN session.
  - **`model`** (two foreign deps): (a) **`provider`** → `provider_reader: ReadPort[Provider]` (used only via `get_or_404` at `:63`). (b) **`model_family`** → depend on `ModelFamilyService`: `_get_family` (`:57`) → `family_service.get_by_id`; the `:187` fallback `resolve_family(self.family_repo.db, id) or self.family_repo.find_first()` → `resolve_family(self.model_repo.db, id) or self.family_service.get_first()` (NB: `resolve_family` is model's OWN helper in `model/lineage.py` — just needs a session, so use model's own `model_repo.db`; add a thin `get_first()` to `ModelFamilyService` wrapping `find_first`). (c) **Enabler:** change `get_or_404(repo: BaseRepository[T])` → `get_or_404(repo: ReadPort[T])` in `base_service.py` so port-typed readers work — all callers pass concrete repos, which satisfy `ReadPort[T]`, so it's compatible/zero-churn. Rewire `model/dependencies.py` to inject `ModelFamilyService` + `ProviderRepository`-as-`ReadPort`.
  - **`chat_session`**→{character, model, profile, persona, message}: analyze per-case (mix of `ReadPort` + service); coupled with `model` — coordinate with **BE-H7**.
  - **`prompt_template`**→`prompt_fragment`: classify each use read-vs-write.
