...
- **Updated:** 2026-07-15
- **Active:** 🔨 **BE-H2 Step 3 in progress** (writes via services). Step 2 complete; `character`→`lore` done (943 green). **To resume:** Step 3 remaining — `st_import`→{preset,profile,fragment,template} (creates across 4 domains) and `chat_session`→`chat_message` greeting seed (deferred from Step 2; sync-safe seam, coordinate w/ BE-H7). Reuse the flush-only-service-method pattern (`FragmentService.delete_orphaned`, `LoreService.import_character_book`). Then **Step 4** (import-boundary AST lint) once all slices are clean. (BE-H1 complete.)
...
- [~] **Step 3 — writes via services** (target slices' published services, not their repos; flush-only methods participate in the caller's UoW — pattern proven by `FragmentService.delete_orphaned`). Watch for service→service cycles.
  ...
  - [ ] **`st_import`→{preset, profile, fragment, template}** (creates across 4 domains).
  - [ ] **`chat_session`→`chat_message`** (greeting seed, deferred from Step 2; needs a sync-safe chat_message seam; coordinate w/ BE-H7).
