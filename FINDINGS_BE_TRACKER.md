...
- **Updated:** 2026-07-15
- **Active:** 🔨 **BE-H2 Step 2 (entangled reads) COMPLETE** — `model`, `prompt_template`, `chat_session` reads all migrated (943 green; chat_session zero test edits). **To resume:** BE-H2 → **Step 3** (writes via services): `character`→`lore`, `st_import`→{preset,profile,fragment,template}, `chat_session`→`chat_message` greeting seed (deferred from Step 2) — reuse the proven flush-only-service-method pattern (`FragmentService.delete_orphaned`). Then **Step 4** (import-boundary AST lint) once all slices are clean. (BE-H1 complete: UoW sync+async across all 17 services.)
...
- [ ] **Step 3 — writes via services:** `character`→`lore` (create lore on card import), `st_import`→{preset, profile, fragment, template} (creates across 4 domains), and `chat_session`→`chat_message` (greeting seed on chat create, deferred from Step 2 — needs a sync-safe chat_message seam; coordinate w/ BE-H7) → depend on the target slices' published services (`LoreService`, etc.), not their repos. Watch for service→service cycles. **Pattern proven in Step 2:** a flush-only service method that participates in the caller's UoW (see `FragmentService.delete_orphaned`).
