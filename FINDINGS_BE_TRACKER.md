...
- **Updated:** 2026-07-15
- **Active:** 🔨 **BE-H2 Step 2 in progress.** `model` + `prompt_template` migrated (943 green). **To resume:** BE-H2 → Step 2 last target `chat_session`→{character, model, profile, persona, message} (per-case `ReadPort`/service; coordinate w/ BE-H7); then Step 3 (writes via services: character→lore, st_import→…) + Step 4 (import-boundary AST lint). (BE-H1 complete: UoW sync+async across all 17 services.)
