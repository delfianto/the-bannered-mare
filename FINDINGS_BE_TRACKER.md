...
- **Updated:** 2026-07-15
- **Active:** ✅ **BE-H2 COMPLETE** (all 4 steps; resolves BE-M11) + **BE-H7 COMPLETE** (Parts A+B). Every in-scope `service.py` is free of foreign `repository.py` imports, enforced by the `tests/test_service_import_boundaries.py` guard (allowlist: `fixtures`, `chat_message→chat_session`). 945 green. **Next up:** BE-M2 (cursor tie-breaker), BE-M3 (pagination constants ⚠ contract), BE-M10 (bookmarks stub ⚠ contract), then BE-H5 (BaseCrudService), BE-H6 (RAG router→service), and BE-L3/L5/L7/L9. Still deferred (need Docker/live PG): BE-H3 part 2, BE-M12(c).
- **Next up:** BE-M2 (cursor tie-breaker), BE-M3 (pagination constants ⚠ contract), BE-M10 (bookmarks stub ⚠ contract), then BE-H5/H6 and the remaining BE-L items.
- **Progress:** 18 / 30 done (BE-H1, **BE-H2**, BE-H4, **BE-H7**, BE-H8, BE-H9, BE-M1, BE-M4, BE-M5, BE-M6, BE-M7, BE-M8, BE-M9, BE-L1, BE-L2, BE-L4, BE-L6, BE-L8 ✓; **BE-M11 resolved by BE-H2**) + BE-H3 part 1 (CI gate) + BE-M12 parts a/b; BE-M12 part c (audit-writer) + BE-H3 part 2 deferred. **Remaining: BE-H5, BE-H6, BE-M2, BE-M3, BE-M10, BE-L3, BE-L5, BE-L7, BE-L9.**
