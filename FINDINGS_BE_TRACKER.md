...
- **Updated:** 2026-07-15
- **Active:** ✅ **BE-H7 DONE** (Part A model↔chat_session snapshot port; Part B greeting-seed seam) + **BE-H2 Steps 2–3 complete**. All in-scope `service.py` files are now free of foreign `repository.py` imports. **Only BE-H2 Step 4 remains** (the import-boundary AST lint) — it must decide fix-vs-allowlist for the 2 remaining out-of-scope importers: `chat_message/service.py`→`chat_session.repository_async` (async read path) and `fixtures/service.py` (startup seeding). 944 green. **Next up after that:** BE-M2 (cursor tie-breaker), BE-M3 (pagination ⚠ contract).
