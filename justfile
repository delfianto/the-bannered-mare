...
# Back up the database to $STORAGE_PATH/backups/<db>-<timestamp>.dump (pg_dump custom format).
[group('database')]
db-backup:
    #!/usr/bin/env bash
    set -euo pipefail
    ...
    mkdir -p "$dir"
    db=$(basename "${url%%\?*}")   # database name from the URL, sans any ?query
    out="$dir/${db}-$(date +%Y%m%d-%H%M%S).dump"
    echo "Backing up → $out"
    pg_dump "$url" --format=custom --file "$out"
    echo "✓ $out ($(du -h "$out" | cut -f1))"

# Restore the database from a dump:  just db-restore storage/backups/<db>-XXXX.dump
[group('database')]
db-restore file:
    #!/usr/bin/env bash
    set -euo pipefail
