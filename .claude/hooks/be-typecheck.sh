#!/usr/bin/env bash
# Stop: enforce the backend QA gate — block finishing while basedpyright reports
# errors in backend/. Runs against $CLAUDE_PROJECT_DIR/backend so it works when
...
if [ "${errs:-0}" -gt 0 ]; then
  printf '%s\n' "$out" | tail -n 40 >&2
  echo "basedpyright reports ${errs} error(s) in backend/ — fix before finishing (backend QA gate)." >&2
  exit 2
fi
exit 0
